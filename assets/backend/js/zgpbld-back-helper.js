...
var zgpbld_back_helper = zgpbld_back_helper || null;
if(!$uifm.isFunction(zgpbld_back_helper)){
(function($, window) {
 "use strict";  
    
var zgpbld_back_helper = function(){
    var zgfm_variable = [];
    ...
    this.filter_content_html =function(str){
      
        var str_output = $('<div/>').html(str);

        //iframe replacement
        var tmp_iframe = str_output.find('iframe').prop('outerHTML');
        str_output.find('iframe').replaceWith($('<div/>').text(tmp_iframe));
    ...
    */
    this.isURIComponentEncoded =function(uri){
      try {
         ...
         uri = uri || '';
          ...
          return uri !== decodeURIComponent(uri);
            ...
            //console.error("error zgpbld_back_helper.isEncoded ", ex.message);
        } 
    ...
    /*
    * htmlEntities equivalent
    */
    this.htmlEntities = function(text) {
      var map = {
    '&': '&amp;',
    '<': '&lt;',
    '>': '&gt;',
...
window.zgpbld_back_helper = zgpbld_back_helper = $.zgpbld_back_helper = new zgpbld_back_helper();
